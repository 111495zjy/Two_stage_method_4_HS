...
def main(args):
    ...
    image_stage1 = model(events.timestamps[0:low_speed_frame_number]).detach().clone()

    #next stage
    print(f'Prepare TS data and event data...')

    event_TS = events.get_TS(events.timestamps[0:low_speed_frame_number],low_speed_frame_number) #event_data.py
